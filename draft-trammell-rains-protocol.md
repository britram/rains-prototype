...
- Bloom filter: A bit string encoding a group of assertions common to a zone,
...
### Shards, Bloom Filters, and Zones {#shards-bloom-filters-zones}
...
A bloom filter is a space-efficient probabilistic data structure represented as
a bit string. It is used to prove membership of an element in a set. The set
could either be the entire zone or an exclusive lexicographic range of that zone
(like the range of a shard). All assertions within the same zone and context,
and whose names are within the range are elements of the set. A bloom filter is
protected by one or more signatures. A membership query to the bloom filter
responds either with 'an assertion with a given name and type might be part of
the set' or 'an assertion with a given name and type is definitely not part of
the set'. The second response can be used to proof nonexistence of an assertion
with a given name and type. There is a tradeoff between the size of the bit
string, membership query time, and the false positive error rate. The zone
authority can determine how to weight them.

A bloom filter has the following information elements:

- Context: name of the context in which the assertions in the bloom filter are
  valid; see {{context-in-assertions}}.
- Zone: name of the zone in which the assertions are made.
- Range: an exclusive lexicographic range within which the contained assertions'
  names must be.
- Hash family: The family of hash function(s) used in the bloom filter.
- Number of Hash functions: number of hash functions used in the bloom filter.
- Filter: a bit string encoding all contained assertions.
- Signatures: one or more signatures generated by the authority for the
  shard; see {{signatures-in-assertions}}.
...
- Content: a set of assertions, bloom filters and/or shards sharing the context
  and zone.
- Signatures: one or more signatures generated by the authority for the
  zone; see {{signatures-in-assertions}}.

For efficiency's sake, information elements within a zone common to all
assertions, shards and bloom filters (zone, context, signature) within the zone
may be omitted from the assertions, shards and bloom filters themselves.

A bloom filter can only be used as a negative answer. A shard should only be
...
Shards, bloom filters and zones can all be used to prove nonexistence during
...
validity period of a shard, bloom filter or zone, leading to an inconsistency.
Thus, a section proving nonexistence only captures the state at the point in
time when it was signed. To make sure that the content of a shard, bloom filter
...
Messages, assertions, shards, bloom filters, zones, queries, and notifications
...
| 11   | range          | Lexical range of Assertions in Shard or Bloom Filter |
...
| 18   | hash-family    | Hash family used in bloom filters                    |
| 19   | nof-hashfunc   | Number of hash functions used in bloom filters       |
| 20   | filter         | bit string encoding assertions used in bloom filters |
...
| 7    | bloomfilter  | Bloom filter (see {{cbor-bloomfilter}})           |
...
range (11) key, since an unsigned contained Shard cannot be used by a RAINS
server to answer a query for nonexistence; it must be returned in a signed Zone.
...
## Bloom Filter body {#cbor-bloomfilter}

A Bloom Filter body is a map. The keys present in the map depend on whether the
Bloom Filter is contained in a Message or in a Zone.

Bloom Filters contained in a Message's content value are "bare Bloom Filters".
Since they cannot inherit any values from their contained Zone, they MUST
contain the filter (20), signatures (0), subject-zone (4), context (6), range
(11), hash-family (18), and nof-hashfunc (19) keys.

Bloom Filters within a Zone are "contained Bloom Filters", and can inherit
values from their containing Zone. A contained Bloom Filters MUST contain the
the filter (20), range(11), hash-family (18), and nof-hashfunc (19) keys. The
subject-zone (4) and context (6) keys MUST NOT be present. They are assumed to
have the same value as the corresponding values in the containing Zone for
signature generation and signature verification purposes; see
{{cbor-signature}}.

A contained Bloom Filter SHOULD contain the signatures (0) key if it also
contains a range (11) key, since an unsigned contained Bloom Filter cannot be
used by a RAINS server to answer a query for nonexistence; it must be returned
in a signed Zone.

The value of the filter (20) key is a bit string as defined in
{{cbor-bloom-filter-bit-string}} according to the hash family used.

The value of the signatures (0) key, if present, is an array of one or more
Signatures as defined in {{cbor-signature}}. If not present, the containing Zone
MUST be signed. Signatures on a contained Bloom Filter are generated as if the
inherited subject-zone and context values are present in the Bloom Filter,
whether actually present or not. The signatures on the Bloom Filter are to be
verified against the appropriate key for the Zone containing the Bloom Filter in
the given context, as described in {{signatures-in-assertions}}.

The value of the subject-zone (4) key, if present, is a UTF-8 encoded string
containing the name of the zone in which the Assertions in the Bloom Filter is
made and MUST end with '.' (the root zone). If not present, the zone of the
assertion is inherited from the containing Zone.

The value of the context (6) key, if present, is a UTF-8 encoded string
containing the name of the context in which the Assertions in the Bloom Filter
...
The value of the range (11) key MUST be a two element array of strings or nulls
(subject-name A, subject-name B). A must lexicographically sort before B, but
neither subject name need be present in the bloom filter's contents. If A is
null, the bloom filter begins at the beginning of the zone. If B is null, the
bloom filter ends at the end of the zone. The bloom filter MUST NOT contain any
assertions whose subject names sort before A or after B.

The value of the hash-family (18) key is an integer specifying a family of hash
function(s) identifier used in the bloom filter, as in {{tabhashfamily}}.

The value of the nof-hashfunc (19) key is an integer determining the number of
hash functions used in the bloom filter.

Bloom Filters are lexicographically complete within the range described in the
range value: a subject-name and type within the range of a bloom filter giving a
...
{: #tabhashfamilies title="Bloom Filter hash families"}

| Code | Name       | Notes                                     |
|-----:|------------|-------------------------------------------|
| 1    | fnv-64     | Value contains FNV-64 hash (64 bytes)     |
| 2    | murmur3-64 | Value contains murmur3-64 hash (64 bytes) |

### Sorting Bloom Filter {#cbor-bloom-filter-sorting}

Bloom Filters are sorted lexicographically by their cbor encoded byte string in
ascending order. That means, they are sorted by the following elements in the
...
{{cbor-shard}}, Bloom Filter bodies as defined in {{cbor-bloomfilter}} and/or
Assertion bodies as defined in {{cbor-assertion}}. Assertions, Bloom Filters and
...
On receipt of an assertion, shard, bloom filter, or zone message section, a
server:
